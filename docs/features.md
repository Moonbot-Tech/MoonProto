...
| Trades and chart history | Tape rows, liquidations, Last/Mark price lines, mini-candles, 5m candles, and maintained analytics. | Subscribe through `client.streams()`; read `snapshot.market_history_readers_for(&market)`. |
...
| Manual strategy mode | Attach a selected strategy to a manual order. `strategy_id = 0` remains a pure manual order. | Read `use_manual_strategy` / `manual_strategy_id`, then call `NewOrderParams::with_strategy_id(...)`. |
