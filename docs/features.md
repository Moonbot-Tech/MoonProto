...
| Headless capture station | Small selected-pair histories, with chart archives on demand and no automatic full-candles download. | `MarketHistorySizing::compact_with_budget_percent(75..=200)`; see [capture lifecycle and RAM estimates](trades.md#compact-capture-stations). |
...
| Manual strategy mode | Choose a strategy per order, independently of the core's Manual trading switch. No id means no strategy on updated cores. | `NewOrderParams::with_strategy_id(...)`; omit it for a bare order. See [core compatibility](trade_actions.md#strategy-selection-and-initial-stops). |
| Initial order stops | Set SL, trailing and TP together with a new order or pending, without changing global settings. | `NewOrderParams::with_stops(...)` / `PendingOrderParams::with_stops(...)`; pass `StopSettings::disabled()` to explicitly turn them off. |
